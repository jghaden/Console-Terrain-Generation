...
namespace TerrainGeneration {
    class Program {
        ...
        protected static int oRow;
        protected static int oCol;

        protected static void PosAt(int x, int y) {
            try {
                Console.SetCursorPosition(oCol + x, oRow + y);
            }
        ...
        public static void Main(string[] args) {
                ...
                if(args[i].StartsWith("-")) {
                    ...
                    switch(optionName) {
                        ...
                        case "help":
                            ...
                            Console.WriteLine("\t-pos\t\tDisplaces the map by the provided amount on X and or Y (default 0x0)");
                        ...
                        case "pos-r":
                            posRest = true;
                            String offsetRText = args[i + 1];

                            int offsetRDelim = offsetRText.IndexOf('x');

                            offX = int.Parse(offsetRText.Substring(0, offsetRDelim));
                            offY = int.Parse(offsetRText.Substring(offsetRDelim + 1));
            ...
            if (posRest) {
                oRow = Console.CursorTop;
                oCol = Console.CursorLeft;
                posRest = false;
            }
